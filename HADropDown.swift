...
class HADropDown: UIView {
    ...
    var itemFont = UIFont.systemFont(ofSize: 10.0)
    ...
    @objc private func didTap(gesture: UIGestureRecognizer) {
        ...
        if !isCollapsed {
            let height : CGFloat = CGFloat(items.count > 5 ? itemHeight*5 : itemHeight*3)
          ...
          rootView?.addSubview(self.table)
           ...
           self.table.reloadData()
            UIView.animate(withDuration: 0.25, animations: { 
                self.table.frame = CGRect(x: self.tableFrame.origin.x, y: self.tableFrame.origin.y + self.frame.height+5, width: self.frame.width+10, height: height)
